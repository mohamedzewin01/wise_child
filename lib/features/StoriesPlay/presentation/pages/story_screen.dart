...
class _StoryScreenState extends State<StoryScreen>
  ...
  void _resetHideTimer() {
    _hideTimer?.cancel();
    // إخفاء الأدوات بعد 3 ثوان من البداية فقط
    _hideTimer = Timer(const Duration(seconds: 3), () {
      if (mounted && _areControlsVisible) {
        _hideControls();
      }
  ...
  // دالة لإنشاء Animation للتبديل بين الصفحات
                ...
                child: BlocConsumer<StoryCubit, StoryState>(
                  ...
                  builder: (context, state) {
                    return GestureDetector(
                      ...
                      // إضافة دعم للسحب الأفقي للتنقل بين الصفحات
                      onHorizontalDragEnd: (details) {
                        const sensitivity = 8.0;
                        if (details.primaryVelocity! > sensitivity) {
                          // سحب لليمين - الصفحة السابقة
                          if (state.currentPage > 0) {
                            context.read<StoryCubit>().pageChanged(state.currentPage - 1);
                          }
                        } else if (details.primaryVelocity! < -sensitivity) {
                          // سحب لليسار - الصفحة التالية
                          if (state.currentPage < widget.storyPages.length - 1) {
                            context.read<StoryCubit>().pageChanged(state.currentPage + 1);
                          }
                        }
                      },
                        ...
                        children: [
                          ...
                          if (_isStoryReady)
                            AnimatedSwitcher(
                              ...
                              child: StoryPageView(
                                key: ValueKey(state.currentPage), // مهم لـ AnimatedSwitcher
                                imageUrl: widget.storyPages[state.currentPage].imageUrl ?? '',
                                text: widget.storyPages[state.currentPage].clipText ?? '',
                          ...
                          // أدوات التحكم السفلية مع أنيميشن
                          Positioned(
                            bottom: 0,
                            left: 0,
                            right: 0,
