...
import 'package:wise_child/features/Home/presentation/widgets/tip_showcase/HomeScreenWithShowcase.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    return ShowCaseWidget(
      onStart: (index, key) {
        // يمكنك إضافة أي منطق هنا عند بدء كل خطوة
      },
      onComplete: (index, key) {
        // يمكنك إضافة أي منطق هنا عند انتهاء كل خطوة
      },
      onFinish: () {
        // حفظ أن المستخدم شاهد الشرح
        CacheService.setData(key: 'home_showcase_seen', value: true);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تم الانتهاء من الشرح! يمكنك الآن استكشاف التطبيق بحرية'),
      ...
      builder: (context) => BlocProvider.value(
        value: viewModel,
        child: const HomeScreenWithShowcase(),
      ),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Showcase(
      ...
      titleTextStyle: const TextStyle(
        ...
        color: Colors.white,
        fontFamily: 'Cairo', // استخدم نفس خط التطبيق
      ),
      ...
      tooltipBackgroundColor: backgroundColor ?? ColorManager.primaryColor,
...
// مساعد لإدارة حالة الشرح
class ShowcaseHelper {
  static const String _showcaseSeenKey = 'home_showcase_seen';
  static const String _showcaseCountKey = 'home_showcase_count';

  static bool hasSeenShowcase() {
    return CacheService.getData(key: _showcaseSeenKey) ?? false;
  }

  static void markShowcaseAsSeen() {
    CacheService.setData(key: _showcaseSeenKey, value: true);
    _incrementShowcaseCount();
  }

  static void resetShowcase() {
    CacheService.setData(key: _showcaseSeenKey, value: false);
  }
  ...
  static bool shouldShowAdvancedTips() {
    return getShowcaseCount() >= 3; // إظهار نصائح متقدمة بعد المشاهدة 3 مرات
  }
}

// إضافة أنيميشن مخصص للشرح
class AnimatedShowcaseWidget extends StatefulWidget {
  final Widget child;
  final Duration animationDuration;

  const AnimatedShowcaseWidget({
    super.key,
    required this.child,
  ...
  @override
  State<AnimatedShowcaseWidget> createState() => _AnimatedShowcaseWidgetState();
}

class _AnimatedShowcaseWidgetState extends State<AnimatedShowcaseWidget>
...
// إضافات مساعدة للتطبيق الرئيسي
class ShowcaseUtils {
// ألوان مخصصة للشرح
  static const List<Color> showcaseColors = [
...
// الحصول على لون عشوائي للشرح
  static Color getRandomShowcaseColor(int index) {
    return showcaseColors[index % showcaseColors.length];
  }

// إنشاء أنيميشن مخصص للشرح
  static Widget createAnimatedShowcase({
...
// إنشاء شرح متاح للجميع
  static Widget createAccessibleShowcase({
...
// حفظ إعدادات الشرح
  static void saveShowcasePreferences(ShowcasePreferences preferences) {
    CacheService.setData(
        key: _showcasePreferencesKey, value: preferences.toMap());
  }
...
// حساب ارتفاع التولتيب
  static double _calculateTooltipHeight(String description,
      List<String>? features, String? tip) {
    double baseHeight = 150;
    baseHeight +=
        (description.length / 50) * 20; // 20 pixels per ~50 characters
